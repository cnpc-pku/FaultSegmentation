#!/usr/bin/env python
# coding: utf-8

# In[1]:


# -*- coding: utf-8 -*-
"""

# Project Name:     Fault Prediction
# File Name:        model2D
# Date:             04/26/2019 9:45 AM
# Using IDE:        PyCharm Community Edition
# Author:           Donglin Zhu
# E-mail:           zhudonglin@cnpc.com.cn
# Copyright (c) 2019, All Rights Reserved.

This is a temporary script file.
"""

import numpy as np
#import matplotlib.pyplot as plt
# import matplotlib.image as mping
import random
#import tensorflow as tf
from scipy import signal
import math
#import rickerwavelet
#from parameters import tindex, vindex, testindex

'''
Matrix=int(input("input matrix size:"))
if Matrix%2==0:
    N=int(Matrix)
else:
    N=int(Matrix+1)
Amp=int(input("input folding amplitude:"))

while Amp*2>N:
    Matrix = int(input("input larger matrix size(please larger than or equal to 2 times of folding amplitude):"))
    if Matrix % 2 == 0:
        N = int(Matrix)
    else:
        N = int(Matrix + 1)
'''
N = 200
C=96
#C1=128
M=20
tindex=5000
vindex=1000
testindex=2000
def ModelGenerator(N,Indexrange,seis_path,labels_path):
    for index in range(Indexrange):
        m1 = np.zeros((N + 1, N + 1))
        m2 = np.zeros((2 * N + 1, N + 1))
        m3 = np.zeros((2 * N + 1, N + 1))
        m4 = np.zeros((2 * N + 1, 2 * N + 1))
        m5 = np.zeros((2 * N + 1, 2 * N + 1))
        m6 = np.zeros((2 * N + 1, 2 * N + 1))
        m7 = np.zeros((2 * N + 1, 2 * N + 1))
        m8 = np.zeros((2 * N + 1, 2 * N + 1))
        m9 = np.zeros((2 * N + 1, 2 * N + 1))

        a = -1 + 2 * np.random.rand(N + 1)
        b = (10 * np.random.rand(M)).astype(np.int)
        # c = (90 * np.random.rand(20)).astype(np.int)
        # b=(np.random.sample(range(1,11),10)).astype(np.int)
        c = random.sample(range(1, (N - 10)), M)

        for i in range(M):
            a[c[i]:c[i] + b[i]] = 0
        x = np.linspace(0, 2 * np.pi, N + 1)
        Amprange = np.array([0, 5, 10, 15, 20, 25, 30])
        Amp = np.random.choice(Amprange, 1)
        # Amp=0
        y = Amp * np.sin(x + 2 * np.pi)  # 50
        ysin = np.rint(y)
        ysin = ysin.astype(np.int)

        # Reflection model
        for i in range(N + 1):
            for j in range(N + 1):
                m1[i][j] = a[i]

        # folding
        for i in range(N + 1):
            for j in range(N + 1):
                m2[i + int(N / 2)][j] = m1[i][j]  # 50 need revise
        # m2[int(N/2):N+1+int(N/2),:]=m1

        for i in range(int(N / 2), int(3 * N / 2 + 1), 1):  # 50,151
            for j in range(N + 1):
                m3[i + ysin[j]][j] = m2[i][j]

        for i in range(2 * N + 1):
            for j in range(N + 1):
                m4[i][j + int(N / 2)] = m3[i][j]  # 50
        # m4[:,int(N / 2):N+1+int(N/2)]=m3


        # fault
        Amp = 30
        krange = [i for i in range(-89, 91)]
        krange = np.array(krange)

        def f(x, N):
            # k = np.random.choice(krange, 1)
            fx = -1 * math.tan(np.pi / 2 - k * np.pi / 180) * x + N * (
                    1 + math.tan(np.pi / 2 - k * np.pi / 180))  # angle [-89,-45]&[45,90]

            ff = np.rint(fx)
            ff = ff.astype(np.int)
            return ff

        kc = np.random.choice(krange, 1)
        # file=open('E:/PyCharm/Practice3/dip.txt','a')
        # file.write(str(k))
        # kc=-25
        # print('kc='+str(kc))
        # print('Amp='+str(Amp))
        if kc in range(0, 45):
            k = kc - 90
            m22 = np.rot90(m4, 1)
        elif kc in range(-45, 0):
            k = kc + 90
            m22 = np.rot90(m4, 1)
        else:
            k = kc
            m22 = m4

        for i in range(2 * N + 1):
            for j in range(f(i, N)):
                m5[i][j] = m22[i][j]
        for i in range(2 * N + 1):
            for j in range(f(i, N), 2 * N + 1, 1):
                m6[i][j] = m22[i][j]

        # displacement=int(input("input fault displacement:"))
        displacement = 15
        dy = displacement / math.tan(abs(k) * np.pi / 180)
        ddy = np.rint(dy)
        ddy = ddy.astype(np.int)
        if k >= 0:
            for i in range(2 * N + 1 - displacement):
                for j in range(2 * N, ddy - 1, -1):
                    m7[i][j] = m5[i + displacement][j - ddy]
            for i in range(2 * N + 1):
                for j in range(2 * N + 1):
                    m9[i][j] = m7[i][j] + m6[i][j]

        else:
            for i in range(2 * N + 1 - displacement):
                for j in range(2 * N + 1 - ddy):
                    m8[i][j] = m5[i + displacement][j + ddy]
            for i in range(2 * N + 1):
                for j in range(2 * N + 1):
                    m9[i][j] = m8[i][j] + m6[i][j]
        # crop
        m10 = np.zeros((2 * C + 1, 2 * C + 1))
        for i in range(2 * C + 1):
            for j in range(2 * C + 1):
                m10[i][j] = m9[i + N - C][j + N - C]  #
        # plt.figure('crop_seis')  # , figsize=(6.4,9.6),dpi=10
        # plt.imshow(m10, cmap=plt.cm.gray)

        # fault_aug
        m26 = m10
        # drange = np.array([[0,0, 5], [0,3, 6], [0,0, 0]])

        # d = random.choice(drange)
        #d = np.array([0, 8, 8, 8])
        if kc in range(-45, 45):
            for i in range(4, 2 * C + 1 - 4):  # 2 * C + 1 - 4
                for j in range(4, 2 * C + 1 - 4):
                    # m26[i-3][f(i,C) - 3] = 6 * m9[i-3][f(i,C) - 3] / 8
                    #m26[i - 3][f(i, C)] = d[3] * m10[i - 3][f(i, C)] / 8
                    #m26[i - 2][f(i, C)] = d[2] * m10[i - 2][f(i, C)] / 8
                    #m26[i - 1][f(i, C)] = d[1] * m10[i - 1][f(i, C)] / 8
                    m26[i][f(i, C)] = 0 #d[0] * m10[i][f(i, C)] / 8
                    #m26[i + 1][f(i, C)] = d[1] * m10[i + 1][f(i, C)] / 8
                    #m26[i + 2][f(i, C)] = d[2] * m10[i + 2][f(i, C)] / 8
                    #m26[i + 3][f(i, C)] = d[3] * m10[i + 3][f(i, C)] / 8
                    # m26[i+3][f(i,C) + 3] = 6 * m9[i+3][f(i,C) + 3] / 8
        else:
            for i in range(4, 2 * C + 1 - 4):  # 2 * C + 1 - 4
                for j in range(4, 2 * C + 1 - 4):
                    #m26[i][f(i, C) - 3] = d[3] * m10[i][f(i, C) - 3] / 8
                    #m26[i][f(i, C) - 2] = d[2] * m10[i][f(i, C) - 2] / 8
                    #m26[i][f(i, C) - 1] = d[1] * m10[i][f(i, C) - 1] / 8
                    m26[i][f(i, C)] = 0 #d[0] * m10[i][f(i, C)] / 8
                    #m26[i][f(i, C) + 1] = d[1] * m10[i][f(i, C) + 1] / 8
                    #m26[i][f(i, C) + 2] = d[2] * m10[i][f(i, C) + 2] / 8
                    #m26[i][f(i, C) + 3] = d[3] * m10[i][f(i, C) + 3] / 8

        if kc in range(-45, 45):
            m23 = np.rot90(m26, -1)
        else:
            m23 = m26
        #print(kc)
        #print(d)
        # plt.figure(figsize=(96,96))


        # convolve
        from scipy import signal
        #rcrange = np.linspace(0.5, 3, 6)  # random choose wavelet
        #rc = np.random.choice(rcrange)
        rc=1.5
        r = signal.ricker(100, rc)
        # plt.figure('wavelet')
        # plt.plot(r)
        m11 = np.zeros((2 * C + 1, 2 * C + 1))
        m20 = np.rot90(m23, 1)
        for i in range(2 * C + 1):
            seis = signal.convolve(m20[i], r, mode='same')  # rickerwavelet.r
            m11[i] = seis
        m21 = np.rot90(m11, -1)

        if kc in range(0, 45):
            k = kc - 90
            m27 = np.rot90(m21, 1)
        elif kc in range(-45, 0):
            k = kc + 90
            m27 = np.rot90(m21, 1)
        else:
            k = kc
            m27 = m21

        d = np.array([0, 8, 8, 8])
        if kc in range(-45, 45):
            for i in range(4, 2 * C + 1 - 4):  # 2 * C + 1 - 4
                for j in range(4, 2 * C + 1 - 4):
                    # m26[i-3][f(i,C) - 3] = 6 * m9[i-3][f(i,C) - 3] / 8
                    # m28[i-3][f(i, C)] = d[3] * m27[i-3][f(i, C) ] / 8
                    # m26[i-2][f(i, C)] = d[2] * m10[i-2][f(i, C) ] / 8
                    # m26[i-1][f(i, C)] = d[1] * m10[i-1][f(i, C)] / 8
                    m27[i][f(i, C)] = 0
                    # m26[i+1][f(i, C)] = d[1] * m10[i+1][f(i, C)] / 8
                    # m26[i+2][f(i, C)] = d[2] * m10[i+2][f(i, C)] / 8
                    # m26[i+3][f(i, C)] = d[3] * m10[i+3][f(i, C)] / 8
                    # m26[i+3][f(i,C) + 3] = 6 * m9[i+3][f(i,C) + 3] / 8
        else:
            for i in range(4, 2 * C + 1 - 4):  # 2 * C + 1 - 4
                for j in range(4, 2 * C + 1 - 4):
                    # m26[i][f(i, C) - 3] = d[3] * m10[i][f(i, C) - 3] / 8
                    # m26[i][f(i, C) - 2] = d[2] * m10[i][f(i, C) - 2] / 8
                    # m26[i][f(i, C) - 1] = d[1] * m10[i][f(i, C) - 1] / 8
                    m27[i][f(i, C)] = 0
                    # m26[i][f(i, C) + 1] = d[1] * m10[i][f(i, C) + 1] / 8
                    # m26[i][f(i, C) + 2] = d[2] * m10[i][f(i, C) + 2] / 8
                    # m26[i][f(i, C) + 3] = d[3] * m10[i][f(i, C) + 3] / 8
        if kc in range(-45, 45):
            m27 = np.rot90(m27, -1)
        else:
            m27 = m27

        # In[9]:

        # Noise
        def wgn(x, snr):
            snr = 10 ** (snr / 10.0)
            xpower = np.sum(x ** 2) / len(x)
            npower = xpower / snr
            return np.random.randn(len(x)) * np.sqrt(npower)

        m28 = np.zeros((2 * C + 1, 2 * C + 1))
        for i in range(2 * C + 1):
            noise = wgn(m27[i], 15)
            m28[i] = m27[i] + noise
            # mn1=np.rot90(m28,-1)
        # m28=mn1

        # crop
        m29 = np.zeros((C, C))
        for i in range(C):
            for j in range(C):
                m29[i][j] = m28[i + C - int(C / 2)][j + C - int(C / 2)]
        np.save(seis_path + str(index) + '.npy', m29)
        # label
        m13 = np.zeros((2 * C + 1, 2 * C + 1))
        for i in range(1, 2 * C):
            for j in range(1, 2 * C):
                #m13[i][f(i, C) - 1] = 1
                m13[i][f(i, C)] = 1
                #m13[i][f(i, C) + 1] = 1
        if kc in range(-45, 45):
            m24 = np.rot90(m13, -1)
        else:
            m24 = m13
        # crop
        m14 = np.zeros((C, C))
        for i in range(C):
            for j in range(C):
                m14[i][j] = m24[i + C - int(C / 2)][j + C - int(C/ 2)]
        #m14=m24[C - int(C / 2):C+C - int(C / 2),C - int(C / 2):C+C - int(C / 2)]
        np.save(labels_path + str(index) + '.npy', m14)
mainpath='images_2'
trains=ModelGenerator(N,tindex,mainpath+'/train_gen/seis/',mainpath+'/train_gen/labels/')
validation=ModelGenerator(N,vindex,mainpath+'/validation/seis/',mainpath+'/validation/labels/')
test=ModelGenerator(N,testindex,mainpath+'/test/seis/',mainpath+'/test/labels/')